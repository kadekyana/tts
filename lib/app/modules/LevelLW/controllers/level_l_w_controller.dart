...
class LevelLWController extends GetxController {
  ...
  Future<void> checkLookAndWriteAnswer(
      String typedAnswer, String correctAnswer, int levelNo, int score) async {
    // Menghapus spasi di awal dan akhir serta memastikan case insensitive
    if (typedAnswer.trim().toLowerCase() ==
        correctAnswer.trim().toLowerCase()) {
      // Jika jawaban benar, tambahkan skor
      await updateLevelScore(levelNo, score);
    } else {
      // Jika salah, kamu bisa memberi feedback atau pengurangan skor jika diinginkan
      print('Jawaban salah');
    }
    update(); // Memperbarui UI setelah verifikasi
  }

  Future<void> fetchScores() async {
    final data = await SQLHelper.getScore('look_and_write');
    scores.clear();
